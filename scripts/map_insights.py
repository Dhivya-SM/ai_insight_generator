...
def generate_action_items(df):
    grouped_actions = {"Negative": [], "Positive": [], "Neutral": []}
    print(f"📊 Unique sentiments found: {df['sentiment'].unique()}")
    ...
    summarized = {
        s: sorted(Counter(actions).items(), key=lambda x: -x[1])
        for s, actions in grouped_actions.items()
    }
    return summarized
